...
(function () {
  ...
  function getInitialTheme() {
    var saved = localStorage.getItem("loficode-theme");
  ...
  if (themeToggleBtn) {
    ...
    themeToggleBtn.addEventListener("click", function () {
      ...
      localStorage.setItem("loficode-theme", next);
      updateThemeIcon();
    });
  }
  ...
  function updateAmbientState() {
    ...
    if (isPlaying && currentSound) {
      ...
      muteToggle.title = "Mudo";
      equalizer.classList.remove("muted");
      ambientLabel.textContent = "Tocando: " + sounds[currentSound].name;
    } else {
      muteToggle.classList.add("muted");
      if (muteIcon) {
        muteIcon.className = "fas fa-volume-mute";
      }
      muteToggle.title = "Tocar sons ambiente";
      equalizer.classList.add("muted");
      if (currentSound) {
        ambientLabel.textContent = "Pausado: " + sounds[currentSound].name;
      } else {
        ambientLabel.textContent = "Clique para iniciar sons ambiente";
      }
    }
  }
    ...
    if (results.length === 0) {
      searchResults.innerHTML = `
        <div class="search-no-results">
          Nenhum post encontrado para "${query}". Tente outras palavras-chave ou navegue pelas tags.
        </div>
      `;
    } else {
  ...
  class LofiCodeSPA {
    ...
    initializeOverlayAmbientControls(overlay) {
      ...
      if (overlayMuteToggle && currentSound) {
        ...
        if (isPlaying) {
          ...
          overlayMuteToggle.title = "Mudo";
          if (overlayEqualizer) overlayEqualizer.classList.remove("muted");
          if (overlayAmbientLabel) overlayAmbientLabel.textContent = `Tocando: ${sounds[currentSound].name}`;
        } else {
          overlayMuteToggle.classList.add("muted");
          if (muteIcon) muteIcon.className = "fas fa-volume-mute";
          overlayMuteToggle.title = "Tocar sons ambiente";
          if (overlayEqualizer) overlayEqualizer.classList.add("muted");
          if (overlayAmbientLabel) overlayAmbientLabel.textContent = `Pausado: ${sounds[currentSound].name}`;
        }
      }
    ...
    updateOverlayAmbientState(overlay) {
      ...
      if (isPlaying && currentSound) {
        ...
        overlayAmbientLabel.textContent = `Tocando: ${sounds[currentSound].name}`;
      } else {
        overlayMuteToggle.classList.add("muted");
        if (muteIcon) {
          muteIcon.className = "fas fa-volume-mute";
        ...
        if (currentSound) {
          overlayAmbientLabel.textContent = `Pausado: ${sounds[currentSound].name}`;
        } else {
          overlayAmbientLabel.textContent = "Clique para iniciar sons ambiente";
        }
      }
    }
  ...
  // Console easter egg
  console.log(`
    ☕ Bem-vindo ao LofiCode! ☕

    Você encontrou o console! Atalhos de teclado:

    'm' - Mudo/desmudo sons ambiente

    Feito com amor, café e muitas interrupções.
    Bom código! ✨
    `);
})();
